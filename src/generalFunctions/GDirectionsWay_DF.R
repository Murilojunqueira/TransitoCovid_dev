...
# Extract data from Google Maps Direction API with waypoints
...
GDirectionsWay_DF <- function(raw_result, CityCode, Cityname, NowTime, Directions = NULL) {
  ...
  QueriesRoutes <- raw_result$routes$bounds %>% 
    ...
    mutate(route = ifelse(route == "", 0, route)) %>% 
  ...
  for (i in seq_len(length(raw_result$routes$legs))) {
    ...
    Routes_NewRow$ViaLat <- raw_result$routes$legs[[i]]$via_waypoint[[1]]$location$lat
    Routes_NewRow$ViaLon <- raw_result$routes$legs[[i]]$via_waypoint[[1]]$location$lng
    Routes_NewRow$ViaStepIndex <- raw_result$routes$legs[[i]]$via_waypoint[[1]]$step_index
    Routes_NewRow$ViaStepInterpolation <- raw_result$routes$legs[[i]]$via_waypoint[[1]]$step_interpolation
